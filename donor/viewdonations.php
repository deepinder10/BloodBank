...
<div class="nav_bg">
 ...
 <div style="height:300px; width:800px; margin:auto; margin-top:50px; margin-bottom:50px; background-color:#f8f1e4; border:2px solid red; box-shadow:4px 1px 20px black;">
   <form method="post" enctype="multipart/form-data">
     ...
     $s="select * from camp,donation where camp.camp_id=donation.camp_id and donation.email='" . $_SESSION["email"] . "' ORDER BY donation.ddate desc";
     $result=mysqli_query($cn,$s);
     $r=mysqli_num_rows($result);
	//echo $r;
     while($data=mysqli_fetch_array($result))
     {
        $Weddingdate = new DateTime($data['9']);
        $date = date_format($Weddingdate, 'd-m-Y');
        echo"<tr><td  style=' padding-left:50px'>$data[1]</td><td  style=' padding-left:50px'>$date</td><td  style=' padding-left:40px'>$data[10]</td><td  style=' padding-left:30px'>$data[12]</td></tr>";
    }
    mysqli_close($cn);
    ?>               
